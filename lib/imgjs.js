/*
	Try on: http://jsfiddle.net/fa5kpz8p/17/
...
(function(window, undefined){
	...
	var encoders = {
		_index: ['none', 'lzw', 'shortcode', 'shortcode2', 'minimify'],
		none: {
		...
		get: function(url){
			...
			var handlers = {
				...
				run: function(){
					...
					while(handlers[this.status].length)
					{
						var handler = handlers[this.status].shift();
						try
						{
