	...
	var encoders = {
		...
		minimify:{
			encode: function(data, options){
				...
				if(options.all || options.comments)
				{
					copy = copy
				...
				if(options.all || options.whitespace)
				{
					copy = copy
				...
				if(options.all || options.unsafe)
				{
					...
					if(options.all_unsafe || options.parse_escapes)
					{
						...
						copy = copy
							...
							.replace(/([^\d])0([0-8]+)/gi, function(_, oct){
								return _ + parseInt(oct, 8);
							})
					...
					if(options.all_unsafe || options.parse_entities)
					{
					...
					if(options.all_unsafe || options.reduce_booleans)
					{
						//Safe for jQuery 2.1.4
						copy = copy
							//Hexadecimal escape -> character
					...
					if(options.all_unsafe || options.eval_constants)
					{
						//Safe for jQuery 2.1.4
						copy = copy
							//.replace(/\s*((?:\d+(?:\.\d*)?|"(?:\\"|[^"])*"|'(?:\\'|[^'])*'^)(?:\s*[+\-*\/%&<>=!]\s*(?:\d+(?:\.\d*)?|"(?:\\"|[^"])*"|'(?:\\'|[^'])*'))+)\s*/gi, function(_){
							.replace(/\s*(\d+(?:\.\d+)(?:\s*(?:[+\-*\/%]|>[>=]|<[<=]|&&?|!(?:==?)?|\|\|?)\s*))\s*/g, function(_){
								try
								{
				...
				if(options.license)
				{
					copy = option.license.toString().replace(/\/*|*\//g,'') + copy;
				}
		...
		shortcode: {
			...
			shortenings:[
				...
				//count: 10
				'undefined', 'null', 'true', 'false', 'Infinity', 'NaN', 'void', 'this', 'arguments', 'self', 
				//count: 17
			...
			/*
				...
				WARNING:
					...
					- The 5th byte stores the encoder index used
						In the future, it will have also the version of the encoder.
						Format: (encoder.version << 4) | encoder.index
						Also, it will have some checks to verify if the image is compatible with the current version
